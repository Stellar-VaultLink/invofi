#![no_std]
use soroban_sdk::{contract, contractimpl, contracttype, symbol_short, Env, Map, Symbol, Address};
...
impl InvoiceRegistryContract {
    ...
    pub fn register_invoice(
        ...
        originator.require_auth(); // Ensure the originator is signing this transaction

        let mut invoices: Map<Symbol, Invoice> = env.storage().persistent().get(&symbol_short!("invoices")).unwrap_or_default();
        ...
        let invoice = Invoice { id, originator, amount, currency, due_date, status: InvoiceStatus::Pending };
    ...
    pub fn get_invoice(env: Env, id: Symbol) -> Invoice {
        let invoices: Map<Symbol, Invoice> = env.storage().persistent().get(&symbol_short!("invoices")).unwrap_or_default();
        invoices.get(&id).unwrap_or_else(|| panic!("Invoice not found"))
    }
    ...
    pub fn update_invoice_status(env: Env, id: Symbol, new_status: InvoiceStatus) -> Invoice {
        let mut invoices: Map<Symbol, Invoice> = env.storage().persistent().get(&symbol_short!("invoices")).unwrap_or_default();
        let mut invoice = invoices.get(&id).unwrap_or_else(|| panic!("Invoice not found"));
