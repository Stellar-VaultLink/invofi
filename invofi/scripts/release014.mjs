...
// Direct buyer-signed release for the VBUC control escrow 014.
const CID = 'CAOYPJKUFDDJAYWRNMF4RS3PD7CPCO2XK2UEJ2ZAQE7FCFSFGXNIOPDL';
...
if (!esc) { console.error('escrow not in read model'); process.exit(1); }
...
const prepared = await s.prepareTransaction(raw);
prepared.sign(buyer);
...
if (sent.status === 'TRY_AGAIN_LATER') sent = await s.sendTransaction(prepared);
